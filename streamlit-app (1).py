...
# CSS para mejorar la visualización en móviles
st.markdown("""
<style>
    @media (max-width: 768px) {
        ...
        /* Mejor espaciado para inputs móviles */
...
# Función para obtener API key (para Streamlit Cloud)
def get_api_key():
    # Primero intentar obtener de secreto de Streamlit
    if 'OPENAI_API_KEY' in st.secrets:
        return st.secrets['OPENAI_API_KEY']
    # Luego intentar obtener de variable de entorno (desarrollo local)
    elif 'OPENAI_API_KEY' in os.environ:
        return os.environ['OPENAI_API_KEY']
    # Si no está disponible, pedirla al usuario
    else:
        ...
        api_key = st.sidebar.text_input(
            ...
            type="password",
            help="Introduce tu API key de OpenAI para usar el servicio."
        )
        ...
        if api_key:
            ...
            st.sidebar.warning("Por favor, introduce una API key para utilizar la aplicación.")
            return None
...
def get_ai_response(prompt, model_choice):
    ...
    if not api_key:
        return "Se requiere una API key para continuar."
...
if option == "Inicio":
    ...
    if st.button("Analizar", use_container_width=True):  # Botón ancho completo para móviles
        if text_input:
    ...
    if st.button("Generar hipótesis", use_container_width=True):
    ...
    if st.button("Generar diseño metodológico", use_container_width=True):
